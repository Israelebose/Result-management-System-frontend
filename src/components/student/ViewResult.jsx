...
export const ViewResult = () => {
  ...
  const [PublishedRes, setPublishedRes] = useState();
      ...
      semCourses.forEach((course, index) => {
        const remark =
          course.Total >= 70
            ...
            : "Poor";
        printWindow.document.write(
          `<tr><td>${index + 1}</td><td>${course.courseCode}</td><td>${
            course.courseTitle
          }</td><td>${course.credits}</td><td>${course.Total}</td><td>${
            course.Grade
          }</td><td>${course.point}</td><td>${remark}</td></tr>`
        );
      });

      printWindow.document.write("</tbody></table>");
      printWindow.document.write(
        `<div class="gpa yellow">Semester GPA <br> ${calculateGPA(
          semCourses
        )}</div>`
      );
    });
  ...
  const downloadResultPDF = () => {
    ...
    semesters.forEach((sem) => {
      const semCourses = resultData?.[sem] || [];
      doc.text(
        `${sem === "first" ? "First" : "Second"} Semester Results`,
        leftMargin,
        y
      );
      y += 8;
      ...
      doc.text(
        `Semester GPA: ${calculateGPA(semCourses)}`,
        leftMargin + 5,
        y + 7
      );
      y += 20;
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(
      `Course Adviser: ${userData.courseAdvisorName || "________________"}`,
      leftMargin,
      y
    );
    y += 8;
  ...
  const isResultPublished = async () => {
    try {
      let semesterParam = selectedSemester;

      // Normalize "first" / "second" → numeric if backend expects 1/2
      if (selectedSemester === "first") semesterParam = 1;
      if (selectedSemester === "second") semesterParam = "all";
      if (selectedSemester === "all") semesterParam = "all";

      const res = await api.get(`/get/results/published`, {
        params: { session: selectedSession, semester: semesterParam },
      });

      // Assume backend returns something like { published: true/false }
      setPublishedRes(res.data);

      return res.data.published;
    } catch (err) {
      console.error("Error checking if results are published:", err);
      setPublishedRes(false);
      return false;
    }
  };

  // console.log(typeof Number(selectedSemester));
    ...
    <div className="min-h-screen ">
      <div className="mx-auto ">
        {userData.is_approved ? (
          <>
            <div className="flex max-sm:flex-col justify-between max-sm:items-center gap-4 mt-5 mb-6 ">
              <div className="flex max-sm:flex-col max-sm:gap-2 max-sm:w-full max-sm:mb-5 gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <select
                    ...
                    onChange={(e) => {setSelectedSemester(e.target.value); isResultPublished();}}
                    className="border rounded px-2 py-1"
                  >
              ...
              <div className="">
                ...
                {(selectedSemester === "first" ||
                  ...
                  <div className="overflow-x-scroll">
                    <h3 className="font-semibold mb-2 ">First Semester</h3>
                        ...
                        <tbody>
                          {firstSemesterResults.map((c, i) => (
                            <tr key={i} className="bg-white border-b text-sm">
                              ...
                              <td className="px-6 py-3">
                                {c.published ? gradeRemark(c.Total) : "-"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <div className="mt-3 bg-yellow-300 inline-block px-4 py-2 rounded">
                      {PublishedRes ? (
                        <div>
                          {PublishedRes.firstSem
                            ? ` Semester GPA: ${calculateGPA(
                                firstSemesterResults
                              )} `
                            : "Semester GPA: Not published yet"}
                        </div>
                      ) : (
                        <div>Semester GPA: Not published yet</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                {(selectedSemester === "second" ||
                  ...
                  <div className="mt-8  overflow-x-auto">
                    <h3 className="font-semibold mb-2 ">Second Semester</h3>
                  ...
                  {PublishedRes ? (
                    <div>
                      {PublishedRes.published
                        ...
                        : "Cumulative GPA: Not published yet"}
                    </div>
                  ) : (
                    <div>Semester GPA: Not published yet</div>
                  )}
